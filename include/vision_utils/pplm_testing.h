 ...
 const std::string & filename_prefix = IMG_DIR "depth/juggling1")
{
  ...
  ppl_utils::Images2PPL file;
  if (!file.convert(filename_prefix))
    return false;
  ppl = file.get_ppl();
  if (nusers > 0 && ppl.poses.size() == 0) {
    printf("pplm_testing: No user found in '%s'\n", filename_prefix.c_str());
    return false;
  }
  if (nusers >= 0) {
    while ((int) ppl.poses.size() > nusers) // remove users if too big
      ppl.poses.pop_back();
    while ((int) ppl.poses.size() < nusers) // clone last user if too small
      ppl.poses.push_back(ppl.poses.back());
  }
  // set foo coordinates
  for (int user_idx = 0; user_idx < nusers; ++user_idx) {
    ppl.poses[user_idx].head_pose.position.x = cos(user_idx*2*M_PI/nusers);
    ppl.poses[user_idx].head_pose.position.y = sin(user_idx*2*M_PI/nusers);
    ppl.poses[user_idx].head_pose.position.z = 1;
  } // end for (user_idx)
  return true;
}
 ...
 const std::string & filename_prefix2 = IMG_DIR "depth/david_arnaud2")
{
  ...
  // check sizes
  ASSERT_TRUE(res.match_success);
  unsigned int ntracks = req.tracks.poses.size(), ncurr_users = req.new_ppl.poses.size();
  ASSERT_TRUE(res.costs.size() == ntracks * ncurr_users)
      << "Costs exp size:" << ntracks * ncurr_users
      << ", real size:" << res.costs.size();
  // solve with LAP
  ...
  ASSERT_TRUE(assignment_utils::linear_assign_from_cost_vec(res.costs, ncurr_users, ntracks, matches, best_cost));
