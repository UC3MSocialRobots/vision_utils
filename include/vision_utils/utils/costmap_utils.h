...
inline bool is_point_in_costmap(const _Pt2 & pt,
  ...
  for (unsigned int cell_idx = 0; cell_idx < costmap.cells.size(); ++cell_idx) {
    ...
    if (fabs(pt.x - costmap.cells[cell_idx].x ) <= halfwidth
      ...
      //      ROS_INFO("Pt (%g, %g) is within the cell of index %i:(%g, %g)+/-(%g, %g)",
      //               pt.x, pt.y, cell_idx,
...
{
  odom_utils::make_trajectory(vel_lin, vel_ang, traj_buffer, time_end, dt,
...
inline float trajectory_mark(const _Pt2 & start_pos, const float & start_yaw,
  ...
  //  return hausdorff_distances::dist_pt_set
  //      (goal, traj_buffer, hausdorff_distances::dist_L2);

  // evaluate distance to the goal - last point
  return hausdorff_distances::dist_L2(goal, traj_buffer.back());
} // end trajectory_mark()


} // end namespace costmap_utils
