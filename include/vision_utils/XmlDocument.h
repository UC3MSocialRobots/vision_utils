...
public:
  ...
  bool load_from_string(const std::string & file_content) {
    //printf("load_from_string('%s')", file_content.c_str());
  ...
  bool load_from_file(const std::string & filename) {
    //printf("load_from_file('%s')", filename.c_str());
    ...
    bool success = retrieve_file(filename, file_content);
    if (!success) {
      printf("Error while retrieving file '%s'. Aborting.\n",
                  filename.c_str());
      return false;
    }
  ...
  void write_to_file(const std::string & filename) const {
    //printf("write_to_file('%s')", filename.c_str());
    save_file(filename, to_string());
  }
  ...
  std::string get_value(const Node* start, const std::string & path) const {
    //printf("get_value('%s')", path.c_str());
    Node* node = get_node_at_direction(start, path);
    if (node == NULL) {
      //printf("The node ''%s'' does not exist in the xml doc", path.c_str());
      return "";
    }
  #if XML_USED_IMPLEMENTATION == XML_IMPLEMENTATION_MXML
    //printf("type:%i", node->type);
    //printf("type:'%s'", node->child->value.text.string);
    // get the child
    node = node->child;
    if (node == NULL) {
      //printf("The node ''%s'' does not have any child", path.c_str());
      return "";
    }
    // get the value
  ...
  Node* get_node_at_direction(const Node* start, const std::string & path) const {
    //printf("get_node_at_direction('%s')", path.c_str());
    if (start == NULL) {
    ...
    for (std::vector<std::string>::const_iterator word = words.begin(); word
      ...
      if (current_node == NULL) {
        //printf("We didin't find the word '%s' in the xml doc", word->c_str());
        return NULL;
      } // end if current == NULL
    } // end loop word
    return current_node;
  ...
  Node* get_node_at_direction(const Node* start,
    ...
    //printf("get_node_at_direction(path:'%s', attribute_name:'%s', attribute_value:'%s')",
    ...
    for(std::vector<Node*>::const_iterator child = children_at_path.begin();
      ...
      //  //printf("child_attribute_value:'%s', attribute_value:'%s'",
  ...
  std::string get_node_attribute(const Node* node, const std::string & attribute) const {
    //printf("get_node_attribute('%s')", attribute.c_str());
    if (node == NULL) {
      //printf("The node is NULL !");
      return "";
    }
    ...
    if (attrib == NULL) {
      //printf("The node does not contain a '%s' attribute !", attribute.c_str());
      return "";
    }
    return attrib->value();
  #endif
  ...
  _T get_node_attribute(const Node* node,
    ...
    bool success;
    _T ans = cast_from_string<_T>(ans_string, success);
  ...
  void get_all_nodes_at_direction(const Node* start, const std::string & path,
    ...
    //printf("get_all_nodes_at_direction('%s')", path.c_str());
  ...
  void get_all_values_at_direction(const Node* start,
    ...
    //printf("get_all_values_at_direction('%s')", path.c_str());
    ...
    for (std::vector<Node*>::const_iterator node = nodes.begin(); node
        ...
        //        bool success;
        //        _T value =
        //                cast_from_string<_T>(son->value.text.string, success);
        //        if (success)
        //            ans.push_back(value);
        ...
        if (value_str != "")
          ans.push_back(cast_from_string<_T>(value_str));
      } // end loop node
  }
  ...
  Node* add_node(Node* father, Node* node_to_add, const std::string & path) {
    //printf("add_node(path:'%s')", path.c_str());
  #if XML_USED_IMPLEMENTATION == XML_IMPLEMENTATION_MXML
    throw std::invalid_argument("Not implemented");
  #elif XML_USED_IMPLEMENTATION == XML_IMPLEMENTATION_RAPID_XML
    if (path == "") {
  ...
  Node* add_node(Node* father, const std::string & node_name, const std::string & path)  {
    //printf("add_node(node_name:'%s', path:'%s')", node_name.c_str(), path.c_str());
  #if XML_USED_IMPLEMENTATION == XML_IMPLEMENTATION_MXML
    throw std::invalid_argument("Not implemented");
  #elif XML_USED_IMPLEMENTATION == XML_IMPLEMENTATION_RAPID_XML
  ...
  void set_node_attribute(Node* node, const std::string & attr_name, const std::string & value) {
    //printf("set_node_value(attr_name:'%s', value:'%s')", attr_name.c_str(), value.c_str());
  #if XML_USED_IMPLEMENTATION == XML_IMPLEMENTATION_MXML
    throw std::invalid_argument("Not implemented");
  #elif XML_USED_IMPLEMENTATION == XML_IMPLEMENTATION_RAPID_XML
    ...
    if (attrib != NULL) {
      //printf("The node already had a '%s' attribute, removing it", attr_name.c_str());
  ...
  void set_node_attribute(Node* node, const std::string & attr_name, const _T & value) {
    std::string value_str = cast_to_string<_T>(value);
