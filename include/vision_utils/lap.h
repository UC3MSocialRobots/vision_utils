...
#include "vision_utils/gnrl.h"
#include <stdio.h>
#include <math.h>

//__attribute__((optimize("-O0"))) // gcc optimization makes it crash
...
*/
double lap(int dim, double **assigncost,
                     ...
                     int *rowsol, int *colsol, double *u, double *v)
{
  ...
  for (i = 0; i < dim; i++)
    for (j = 0; j < dim; j++)  {
      ...
      {
        printf("\n");
        printf("negative reduced cost i %d j %d redcost %g\n", i, j, redcost);
        printf("\n\ndim %5d - press key\n", dim);
        // scanf("%d", &wait);
        break;
      }
  ...
  for (i = 0; i < dim; i++) {
    ...
    if (fabs(redcost) > 1E-2)
    {
      printf("\n");
      printf("non-null reduced cost i %d soli %d redcost %g\n", i, rowsol[i], redcost);
      printf("\n\ndim %5d - press key\n", dim);
      // scanf("%d", &wait);
      break;
    }
