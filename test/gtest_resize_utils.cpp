...
void benchmark(const std::string & imgfile,
  ...
  for (unsigned int time = 0; time < ntimes; ++time)
    image_utils::scale_img_forward_backward(img, img_out, xscale, xoffset, yscale, yoffset, true);
  timer.printTime_factor("scale_img_forward()", ntimes);

  timer.reset();
  for (unsigned int time = 0; time < ntimes; ++time)
    image_utils::scale_img_forward_backward(img, img_out, xscale, xoffset, yscale, yoffset, false);
  timer.printTime_factor("scale_img_backward()", ntimes);

  timer.reset();
  for (unsigned int time = 0; time < ntimes; ++time)
    image_utils::scale_img_warp(img, img_out, xscale, xoffset, yscale, yoffset);
